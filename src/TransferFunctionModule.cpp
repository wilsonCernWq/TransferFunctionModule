...
// ======================================================================== //
//static simple_opacity_list_t test0 = {
//    {
//        //{ 0.f, std::make_shared<simple_opacity_control_t>(0.1f) },
//        { 0.4f, std::make_shared<simple_opacity_control_t>(0.4f) },
//        { 0.5f, std::make_shared<simple_opacity_control_t>(0.5f) },
//    }
//};
//static bool testfcn() {
//    //test0.insert(0.f, std::make_shared<simple_opacity_control_t>(0.2f) );
//    test0.insert(0.5f, std::make_shared<simple_opacity_control_t>(0.4f) );
//    std::cout << "size " << test0.size() << std::endl;
//    std::vector<opacity_t> output(101);
//    test0.lerp(output);
//    std::cout << "final result" << std::endl;
//    for (size_t i = 0; i < output.size(); ++i)
//        std::cout << (float)i / (output.size()-1) << " " << output[i].a << std::endl;
//    
//    std::cout << "find 0.2 " << test0.find(0.1) << std::endl;
//    return true;
//}
//static bool v = testfcn();
...
name(name), rgbValues(rgbValues), opacityValues(opacityValues),
dataValueMin(dataValueMin), dataValueMax(dataValueMax),
opacityScaling(opacityScaling)
{}
...
ColorPoint::ColorPoint(const float cp, const float cr, const float cg, const float cb)
: p(cp), r(cr), g(cg), b(cb) {}
...
OpacityPoint_Linear::OpacityPoint_Linear(const float cp, const float ca) : p(cp), a(ca) {}
OpacityPoint_Linear::OpacityPoint_Linear(const OpacityPoint_Linear& c) : p(c.p), a(c.a) {}
OpacityPoint_Linear& OpacityPoint_Linear::operator=(const OpacityPoint_Linear &c) {
    if (this == &c) { return *this; }
    p = c.p; a = c.a;
    return *this;
}

OpacityPoint_Gaussian::OpacityPoint_Gaussian(const float cp,  const float ca,
                                             const float csw, const float csy,
                                             const float cbx, const float cby)
: p(cp), a(ca), sw(csw), sy(csy), bx(cbx), by(cby) {}
OpacityPoint_Gaussian::OpacityPoint_Gaussian(const OpacityPoint_Gaussian& c)
: p(c.p), a(c.a), sw(c.sw), sy(c.sy), bx(c.bx), by(c.by) {}
OpacityPoint_Gaussian& OpacityPoint_Gaussian::operator=(const OpacityPoint_Gaussian &c) {
    if (this == &c) { return *this; }
    p = c.p; a = c.a; sw = c.sw; sy = c.sy; bx = c.bx; by = c.by;
    return *this;
}
